...
namespace Wsqm
{
    ...
    internal class Decoder
    {
        ...
        //}
        public BitmapWithMetadata decode(DataInput dataInput)
        {
            try
            {
                ...
                int marker = getCMarkerWSQ(dataInput, WsqConstants.TBLS_N_SOF);
                while (marker != WsqConstants.SOF_WSQ)
                {
                    getCTableWSQ(dataInput, token, marker);
                    marker = getCMarkerWSQ(dataInput, WsqConstants.TBLS_N_SOF);
                }
                ...
                nistcom.Remove(NistConstants.NCM_HEADER);
                nistcom.Add(NistConstants.NCM_PIX_WIDTH, width.ToString());
                nistcom.Add(NistConstants.NCM_PIX_HEIGHT, height.ToString());
                nistcom.Add(NistConstants.NCM_PIX_DEPTH, "8");
                nistcom.Add(NistConstants.NCM_LOSSY, "1");
                nistcom.Add(NistConstants.NCM_COLORSPACE, "GRAY");
                nistcom.Add(NistConstants.NCM_COMPRESSION, "WSQ");

                //nistcom.remove(NistConstants.NCM_HEADER);
                //nistcom.put(NistConstants.NCM_PIX_WIDTH, Integer.toString(width));
                //nistcom.put(NistConstants.NCM_PIX_HEIGHT, Integer.toString(height));
                //nistcom.put(NistConstants.NCM_PIX_DEPTH, "8");
                //nistcom.put(NistConstants.NCM_LOSSY, "1");
                //nistcom.put(NistConstants.NCM_COLORSPACE, "GRAY");
                //nistcom.put(NistConstants.NCM_COMPRESSION, "WSQ");
                bool ppiOk = false;
                try
                {
                    if (Convert.ToInt32(nistcom[NistConstants.NCM_PPI]) > 0)
                        ppiOk = true;

                    //if (Integer.parseInt(nistcom.get(NistConstants.NCM_PPI)) > 0)
                    //    ppiOk = true;
                }
                catch (Exception) { }
                if (!ppiOk)
                    nistcom.Add(NistConstants.NCM_PPI, "-1");

                //nistcom.put(NistConstants.NCM_PPI, "-1");
                return new BitmapWithMetadata(cdata, width, height, Convert.ToInt32(nistcom[NistConstants.NCM_PPI]), 8, 1, nistcom, comments.ToArray());

                //return new cBitmapWithMetadata(cdata, width, height, Integer.parseInt(nistcom.get(NistConstants.NCM_PPI)), 8, 1, nistcom, comments.toArray(new String[0]));
            }
            catch (IOException e)
            {
                throw (e);
        ...
        private KeyValuePair<String, String> stringToFet(String comment)
        {
            try
            {
                if (!comment.StartsWith(NistConstants.NCM_HEADER))
        ...
        public void getCHuffmanTableWSQ(DataInput DataInput, Token token)
        {
            try
            {
                ...
                WsqHelper.HuffmanTable firstHuffmanTable = getCHuffmanTable(DataInput, token, WsqConstants.MAX_HUFFCOUNTS_WSQ, 0, true);
                ...
                while (bytesLeft != 0)
                {
                    ...
                    WsqHelper.HuffmanTable huffmantable = getCHuffmanTable(DataInput, token, WsqConstants.MAX_HUFFCOUNTS_WSQ, bytesLeft, false);
        ...
        private int[] huffmanDecodeDataMem(DataInput DataInput, Token token, int size)
        {
            try
            {
                ...
                Ref<int> marker = new Ref<int>(getCMarkerWSQ(DataInput, WsqConstants.TBLS_N_SOB));
                ...
                while (marker.value != WsqConstants.EOI_WSQ)
                {
                    if (marker.value != 0)
                    {
                        while (marker.value != WsqConstants.SOB_WSQ)
                        {
                            ...
                            marker.value = getCMarkerWSQ(DataInput, WsqConstants.TBLS_N_SOB);
                        }
